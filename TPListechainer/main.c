...
debut:
    ...
    case 6:
        ...
        scanf("%d", &p);
        while (p > (Longeurliste2(liste) + 1))
        {
            printf("la liste contient %d elements, impossible d'inserer a la position %d \n\tEntrer a nouveau la position :");
            scanf("%d", &p);
        }
    ...
    case 9:
        ...
        scanf("%d", &p);
        liste = suppressionMlieu(liste, p);
        break;

    case 10:
        printf("Entrer la valuer de l element a chercher \t");
        scanf("%d", &element);
        chercherelemen(liste, element);
        break;

    case 11:
        liste = Listetrier(liste);
        break;

    case 12:
        printf("Entrer la valuer a ajouter\t");
        scanf("%d", &element);
        liste = ajoutBonendroi(liste, element);
        break;

    case 13:
        liste = ElimiDoublon(liste);
        break;

    default:
        printf("Erreur de saisie \t entere une bonne valeur ");
    }

    printf("Avez-vous fini ? vous voulez quitter ? \t 1 pour oui et 0 pour non ? ");
